...
Function Check-For-GC-Log {
	if (GC_LOG){
        ...
        if ($JAVA_9_PLUS -eq $true)
        {
            $args = (,"-Xlog:gc*:file=%JBOSS_LOG_DIR%\gc.log:time,uptimemillis:filecount=5,filesize=3M",'-version')
        }
        else
        {
            $args = (,"-Xloggc:$JBOSS_LOG_DIR/gc.log","-XX:+PrintGCDetails","-XX:+PrintGCDateStamps","-XX:+UseGCLogFileRotation","-XX:NumberOfGCLogFiles=5","-XX:GCLogFileSize=3M","-XX:-TraceClassUnloading",'-version')
        }
